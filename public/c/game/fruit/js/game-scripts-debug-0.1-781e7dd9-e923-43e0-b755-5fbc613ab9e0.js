...
var MJ = qc.defineBehaviour('qc.engine.MJ', qc.Behaviour, function() {
}, {
    coinAtlas: qc.Serializer.TEXTURE
});

//初始化
MJ.prototype.awake = function() {
    for (var i = 0; i < 10; i++) {
            //创建Sprite
          ...
          // sprite.x = this.game.math.random(0, this.gameObject.width);
            //sprite.y = this.gameObject.y;
            sprite.x = this.gameObject.width+(i*sprite.width)+5;
            sprite.addScript("qc.demo.MjEvent");
        	sprite.speed = sprite.speed + (i*0.4);  

    }

    // 播放帧动画
    this.gameObject.children.forEach(function(child) {        
       // child.playAnimation('play_1');
        
    });
};
var MjEvent = qc.defineBehaviour('qc.demo.MjEvent', qc.Behaviour, function() {
    this._count = 0;   
    this._speed = 0;
   
}, {   
    play_sound: qc.Serializer.Node,
 ...
 * 初始化处理
 */
MjEvent.prototype.awake = function() {
    var self = this,
        o = self.gameObject;
    ...
    o.onLoopFinished.add(function() {
      //var t = self.game.timer.add(2000, function() {
         
         self._count++;
         var p = self.game.math.random(1, 12);
         //var speed = self.game.math.random(1,1);
         o.playAnimation('play_'+p, self.speed, true);        
     // });
    ...
    // 开始播放动作
             var p = self.game.math.random(1, 12);
         var speed = self.game.math.random(0.5, 1);
     o.playAnimation('play_'+p,self.speed, true);
};
 ...
 * 帧调度
 */
MjEvent.prototype.update = function() {
...
var Online = qc.landlord.Online = function() {
    ...
    var self = this;
    //self.socket = io.connect('119.147.137.155:8081',{'connect timeout': 10000});
    self.socket = io.connect('localhost:8081',{'connect timeout': 10000});
    self.init();
};
...
Online.prototype.init = function(){
    ...
    //房间幸运数
    self.socket.on('luckyNumber', function(data){
        console.info('luckyNumber',data);
        //window.roomPlayUI.showResult(data)
       
             window.dashBoard.showResult(data)
    ...
    //倒数N秒
    self.socket.on('counter', function(data){ 
             window.counter.reset(data.second)            
    });
 ...
 * 
 */
Online.prototype.queryDeskMgr = function (){
    var self = this;
    return new Promise(function(resolve, reject){
        try {
            self.socket.emit('queryDeskMgr');
            //注册结果
            self.socket.on('queryDeskMgrResult', function(data){
 ...
 * @param  {[type]} nickname 昵称
 */
Online.prototype.joinRoom = function (rid,uid){
    console.log("joinRoom",rid,uid)
    var self = this;
    return new Promise(function(resolve, reject){
        try {
             self.socket.emit('joinRoom', {'rid':rid,'uid' : uid});
            //加入游戏结果，获取桌位信息
            self.socket.on('joinRoomResult', function(data){
                console.log(data);
                resolve(data);
            });
...
var MainUI = qc.defineBehaviour('qc.engine.MainUI', qc.Behaviour, function() {
    ...
    joinRoomBtn : qc.Serializer.NODE,
    roomScene: qc.Serializer.STRING,
...
MainUI.prototype.awake = function() {
    var self = this;
            ...
            var result = G.online.queryDeskMgr();
            result.then(function(data){
                console.log("queryDeskMgr",data.desks['tb2']);
            }).catch(function(err){
    ...
    //进入指定房间
    self.addListener(self.joinRoomBtn.onClick, function(){
        var rid="tb2";
        var uid = G.game.storage.get('uid');
        if(uid){
            self.showMessage(self.MSG_WAITING);
            self.joinRoom(rid,uid);
        } else {
...
MainUI.prototype.joinRoom = function(rid,uid) {
    var self = this;
    G.ownPlayer = new qc.landlord.Player(null);
    G.ownPlayer.uid = uid;
    window.onbeforeunload = function (){
    ...
    G.online.joinRoom(rid,G.ownPlayer.uid).then(function(data){
        var player = data.seats ? data.seats[data.ownSeatNo] : data.desk.seats[data.ownSeatNo];
        G.ownPlayer.deskNo = player.deskNo;
        G.ownPlayer.name = player.name;
        G.ownPlayer.seatNo = data.ownSeatNo;
        G.netInitData = data;
        self.game.state.load(self.roomScene, false, function() {
        }, function() {
            console.log(self.roomScene + '场景加载完毕。');
        });
    });
};
...
var Counter = qc.defineBehaviour('qc.engine.Counter', qc.Behaviour, function() {
     window.counter=this;
    this.counter = -1;
    this.sound = null;
    this.sound2 = null;   
    this.ts = null;   
     ...
     sounds: qc.Serializer.AUDIOS,     
     atlas: qc.Serializer.TEXTURE,
     ligth: qc.Serializer.TEXTURE
});

Counter.prototype.awake = function() {    
    var self = this, o = self.gameObject;
    //self.texture = this.atlas;
    o.visible = false;
    var ts = o.addScript('qc.TweenScale');  
    ...
    ts.curve.keys = [new qc.Keyframe(0, 0, 0, 0),        
        new qc.Keyframe(0.5, 0.5, 0, 0), 
        new qc.Keyframe(1, 0, 0, 0), 
        new qc.Keyframe(0,0)];     
    if (this.atlas){
        self.texture = this.atlas;
    }   
    ...
    var sound2 = self.game.add.sound();
    sound2.destroyWhenStop = false;
    sound2.audio = self.sounds[1];
    sound2.loop = false;
    sound2.volume = 0.5;    
    this.sound2 = sound2;
...
Counter.prototype.reset = function(time) {
  var self = this;
  this.counter = 3;
...
Counter.prototype.updateCounter = function() {
    ...
    var self = this,    o = self.gameObject , p =this.counter ;
...
var DashBoard = qc.defineBehaviour('qc.engine.DashBoard', qc.Behaviour, function() {
...
DashBoard.prototype.onEnable = function() {
    ...
    var sprite = this.game.add.sprite(this.gameObject);
    sprite.texture = this.atlas;
    sprite.frame ="9.png";
    this.sprite = sprite;

    var position = this.game.add.sprite(this.gameObject);
    position.texture = this.atlas;
    position.frame ="9.png";
...
this.firstTime = true;
var pos= [[0,0,1],[1,0,2],[2,0,3],[3,0,4],[4,0,5],[5,0,6],[6,0,7],        
          [6,1,1],[6,2,2],[6,3,3],[6,4,4],[6,5,5],
          [6,6,1],[5,6,2],[4,6,3],[3,6,4],[2,6,5],[1,6,6],[0,6,7],
          [0,5,1],[0,4,2],[0,3,3],[0,2,4],[0,1,5]
         ]
this.pos= pos;
 if (!this.bingo ){
  var sprite = this.game.add.sprite(this.gameObject);
    sprite.texture = this.light;
    sprite.frame ="bingo-62.png";
    sprite.x = 0;
    sprite.y = 0;

    this.bingo = sprite ;
    this.bingo.resetNativeSize(); 
this.bingo.visible = false;
 }
...
DashBoard.prototype.onClick = function() {
    var result ={}
    result.fruit = 5
    this.showResult(result)

};
DashBoard.prototype.showResult = function(result) {
 this.bingo.visible = true;
 if (!this.position ){
  var sprite = this.game.add.sprite(this.gameObject);
    sprite.texture = this.atlas;
    sprite.frame ="0.png";
    ...
    var tp =  this.position.addScript('qc.TweenPosition');
    tp.curve.keys = [new qc.Keyframe(0, 0, 0, 0), 
        
        new qc.Keyframe(0.18, 0.635,1.48,1.48), 
        new qc.Keyframe(1, 1,0,0), 
        new qc.Keyframe( 0, 0)];
...
DashBoard.prototype.update = function() {
  /*
   var p = Math.floor(this.gameObject.y / 10)
   console.log(p)
   this.sprite.frame =p+".png";
   */
   if (this.position){
       if (this.position.x){
...
// define a user behaviour
var RoomPlayUI = qc.defineBehaviour('qc.engine.RoomPlayUI', qc.Behaviour, function() {
    // need this behaviour be scheduled in editor
    //this.runInEditor = true;
    this.own = null;
    this.left = null;
...
// Called when the script instance is being loaded.
RoomPlayUI.prototype.awake = function() {
    var self = this;
    self.init();
    /*
    //离开按钮
 ...
 * @return {[type]} [description]
 */
RoomPlayUI.prototype.init = function (){
    this.testJoinRoom();
...
//更新桌位信息
RoomPlayUI.prototype.refreshInfo = function (seats){
    var self = this;
    if(G.ownPlayer.seatNo == 'p1'){
        self.setRightInfo(seats.p2);
        self.setLeftInfo(seats.p3);
...
//设置左边玩家信息
RoomPlayUI.prototype.setLeftInfo = function (player){
    var self = this;
    if(player){
        self.left.getScript('qc.engine.OPlayerUI').scorePanel.text = player.score + '';
        if(G.leftPlayer && G.leftPlayer.name == player.name) return;
...
//设置右边玩家信息
RoomPlayUI.prototype.setRightInfo = function (player){
    var self = this;
    if(player){
        self.right.getScript('qc.engine.OPlayerUI').scorePanel.text = player.score + '';
        if(G.rightPlayer && G.rightPlayer.name == player.name) return;
...
//清空自己出牌区域
RoomPlayUI.prototype.cleanMyPlayArea = function(){
    var self = this;
    if(self.myPlayArea.children.length > 0){
        for (var i = 0; i < self.myPlayArea.children.length; i++) {
            self.myPlayArea.children[i].destroy();
...
//清空左边玩家出牌区域
RoomPlayUI.prototype.cleanLeftPlayArea = function(){
    var self = this;
    if(self.leftPlayArea.children.length > 0){
        for (var i = 0; i < self.leftPlayArea.children.length; i++) {
            self.leftPlayArea.children[i].destroy();
...
//清空右边玩家出牌区域
RoomPlayUI.prototype.cleanRightPlayArea = function(){
    var self = this;
    if(self.rightPlayArea.children.length > 0){
        for (var i = 0; i < self.rightPlayArea.children.length; i++) {
            self.rightPlayArea.children[i].destroy();
 ...
 */
 /*
RoomPlayUI.prototype.playerReady = function (data){
    var self = this,
        area = null;
    if(G.leftPlayer && G.leftPlayer.seatNo === data.seatNo){
        area = self.leftPlayArea;
...
//重置桌位信息
RoomPlayUI.prototype.reset = function(){
    var self = this;
    //隐藏桌面信息
    self.readyBtn.visible = false;
    self.cleanMyPlayArea();
 ...
 * @return {[type]}      [description]
 */
RoomPlayUI.prototype.start = function(data){
    var self = this;
    self.reset();
    //发牌动画
    var idx = 0, toatl = data.cardList.length;
 ...
 * @return {[type]}                [description]
 */
RoomPlayUI.prototype.insertCardForOther = function (area, n){
    var self = this;
    n = n ? n : 1;
    for (var i = 0; i < n; i++) {
        var c = self.game.add.clone(self.cardPrefab, area);
 ...
 * @return {[type]}      [description]
 */
RoomPlayUI.prototype.insertOneCard = function (card){
    var self = this,
        insertIndex = 0;
    if(G.currentCards.length != 0 && G.gameRule.cardSort(card, G.currentCards[0].getScript('qc.engine.CardUI').info) === 1){//比第一张牌小才需要查询
        for (var j = 0; j < G.currentCards.length; j++) {//查询新的牌应放置的位置
 ...
 * @return {[type]}              [description]
 */
RoomPlayUI.prototype.robLandlord = function(seatNo, currentScore, preSeatNo, preScore){
    var self = this;
    if(G.ownPlayer.seatNo === seatNo){
        self.game.add.clone(self.clockPrefab, self.myPlayArea);
        //添加一个计时器，30秒没有操作叫分，默认选中不叫
...
RoomPlayUI.prototype.showRobScore = function(seatNo, score){
    var self = this, area = null;
    if(seatNo === G.ownPlayer.seatNo){
        area = self.myPlayArea;
    } else if(seatNo === G.leftPlayer.seatNo){
...
//显示叫分按钮
RoomPlayUI.prototype.showRobBtns = function(currentScore){
    var self = this;
    self.scoreZero.visible = true;
    self.scoreThree.visible = true;
    if(currentScore < 2)
...
//隐藏叫分按钮
RoomPlayUI.prototype.hideRobBtns = function(){
    var self = this;
    self.scoreZero.visible = false;
    self.scoreThree.visible = false;
    self.scoreTwo.visible = false;
 ...
 * @return {[type]}             [description]
 */
RoomPlayUI.prototype.playerProvideScore = function (s){
    var self = this;
    self.cleanMyPlayArea();
    //删除定时器
    if(self.robTimer){
...
//显示底牌
RoomPlayUI.prototype.showHiddenCards = function(){
    var self = this;
    for (var i = 0; i < self.hiddenContainer.children.length; i++) {
        self.hiddenContainer.children[i].frame = G.hiddenCards[i].icon;
    }
 ...
 * @return {[type]}      [description]
 */
RoomPlayUI.prototype.setLandlord = function(data){
    var self = this;
    if(data.preSeatNo && data.preSeatNo != G.ownPlayer.seatNo){
        self.showRobScore(data.preSeatNo, data.preScore);
    }
 ...
 * 显示玩家剩余牌
 */
RoomPlayUI.prototype.showPlayerCards = function (leftCardList, rightCardList){
    var self = this,
        leftCantainer = self.left.getScript('qc.engine.OPlayerUI').cardContainer,
        rightCantainer = self.right.getScript('qc.engine.OPlayerUI').cardContainer;
    for (var i = 0; i < leftCardList.length; i++) {
 ...
 * 游戏中有强制退出的
 */
RoomPlayUI.prototype.forceExit = function (data){
    var self = this;
    //清空提示信息
    if(data.status === 2){//抢地主过程退出
        self.reset();
 ...
 * 游戏中有强制退出的
 */
RoomPlayUI.prototype.playerBack = function (data){
    var self = this;
    if(G.leftPlayer.seatNo === data.seatNo){
        G.leftPlayer.isLeave = false;
        self.left.getScript('qc.engine.OPlayerUI').namePanel.color = new qc.Color(0xffffa500);
...
RoomPlayUI.prototype.testJoinRoom = function() {
    console.log("testJoinRoom")
    var self = this;
    /*
    G.ownPlayer = new qc.landlord.Player(null);
    G.ownPlayer.uid = uid;
    ...
    */
    ...
    var uid = G.game.storage.get('uid');
    if(uid){
       // G.mainUI.showMessage(self.MSG_WAITING);
      //  window.mainUI.joinRoom(rid,uid);

        G.online.joinRoom(rid,uid).then(function(data){
            /*
            var player = data.seats ? data.seats[data.ownSeatNo] : data.desk.seats[data.ownSeatNo];
            G.ownPlayer.deskNo = player.deskNo;
            G.ownPlayer.name = player.name;
...
RoomPlayUI.prototype.showResult = function(data) {
    console.log("showResult",data)
}
RoomPlayUI.prototype.WAITING = '等待加入';
RoomPlayUI.prototype.LEAVE = ' 离开了';
RoomPlayUI.prototype.BACK = ' 回来了';
RoomPlayUI.prototype.ENTER = ' 加入了游戏';
RoomPlayUI.prototype.READY = '准备';
RoomPlayUI.prototype.CANCEL_READY = '取消准备';
